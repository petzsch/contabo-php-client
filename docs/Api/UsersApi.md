...
```php
getObjectStorageCredentials($x_request_id, $user_id, $x_trace_id): \OpenAPI\Client\Model\CredentialResponse
```
...
try {
    $result = $apiInstance->getObjectStorageCredentials($x_request_id, $user_id, $x_trace_id);
    print_r($result);
} catch (Exception $e) {
...
```php
regenerateCredentials($x_request_id, $user_id, $x_trace_id): \OpenAPI\Client\Model\CredentialResponse
```
...
try {
    $result = $apiInstance->regenerateCredentials($x_request_id, $user_id, $x_trace_id);
    print_r($result);
} catch (Exception $e) {
