...
$size = 10; // int | Number of elements per page.
$order_by = name:asc; // string[] | Specify fields and ordering (ASC for ascending, DESC for descending) in following format `field:ASC|DESC`.
$data_center_name = EU; // string | Filter for Object Storage locations.
...
```php
upgradeObjectStorage($x_request_id, $object_storage_id, $upgrade_object_storage_request, $x_trace_id): \OpenAPI\Client\Model\UpdateObjectStorageResponse
```
...
[**\OpenAPI\Client\Model\UpdateObjectStorageResponse**](../Model/UpdateObjectStorageResponse.md)
